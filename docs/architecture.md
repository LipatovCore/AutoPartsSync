...
- `employees.views`
  Содержит `EmployeeLoginView`, admin-only view для списка сотрудников, создания сотрудника, перевыпуска приглашения и деактивации сотрудника, а также публичный view установки пароля по invite-токену.
...
- `employees.services.access_service`
  Реализует административный сценарий деактивации сотрудника: переводит учетную запись в `deactivated`, завершает активные сессии, пишет событие аудита и запрещает системному администратору деактивировать собственную учетную запись.
...
- `employees.admin`
  В Django admin зарегистрирован журнал аудита доступа для просмотра системными администраторами.
